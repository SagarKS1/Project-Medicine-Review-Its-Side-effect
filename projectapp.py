...
def main():
        ...
        output = st.text_input("Enter Your Illness Condition") 
        with st.beta_expander("Please find the drug details below", expanded=True):
                retrived_df = df[df["condition"].str.contains(output, na=False)]
                                 
        st.dataframe(retrived_df[["condition", "drugName"]])
        ...
        if choice == "Single":
            ...
            drug = st.selectbox("DrugName List", drug_list)
